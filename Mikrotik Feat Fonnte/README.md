__Send Message Fonnte Via Terminal__ :speech_balloon:
```
...
```
__Netwatch Check Status UP__ :heavy_check_mark:
```
:local hh $host
:local wa "@nohp"
...
```
__Netwatch Check Status DOWN__ :x:
```
:local hh $host
:local wa "@nohp"
:local datetime "Tanggal:%20$[/system clock get date]%0AJam:%20$[/system clock get time]"
...
```
__PPPOE Profile Script Status UP__ :heavy_check_mark:
```
:local nama "$user";
:local wa "@nohp";
...
```
__PPPOE Profile Script Status DOWN__ :x:
```
:local wa "@nohp";
:local lastdisc [/ppp secret get [find name=$user] last-disconnect-reason];
:local lastlogout [/ppp secret get [find name=$user] last-logged-out];
